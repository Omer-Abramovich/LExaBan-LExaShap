...
from .ShapleyEngine import Value
import time
...
def exc_or(dnf):
  ...
  total_vars = set.union(*[item.vars for clause in dnf for item in clause])
  ...
  ### When we need to follow assignment sizes, they also depend on the variables that we dropped in the subset clauses
  ### We need to count these variables and account for them in the probability. 
  true_branch_vars = set.union(*[item.vars for clause in true_dnf for item in clause])
  if len(total_vars) > len(true_branch_vars) + len(max_fact.vars):
     new_prob = Value._all_assignments_by_size(len(total_vars) - len(true_branch_vars) - len(max_fact.vars))
     new_vars = total_vars - true_branch_vars - max_fact.vars
     added_value = Value(prob= new_prob, vars=new_vars)
     true_dnf = [clause.union({added_value}) for clause in true_dnf]

  false_branch_vars = set.union(*[item.vars for clause in false_dnf for item in clause])
  if len(total_vars) > len(false_branch_vars) + len(max_fact.vars):
     new_prob = Value._all_assignments_by_size(len(total_vars) - len(false_branch_vars) - len(max_fact.vars))
     new_vars = total_vars - false_branch_vars - max_fact.vars
     added_value = Value(prob= new_prob, vars=new_vars)
     false_dnf = [clause.union({added_value}) for clause in false_dnf]
